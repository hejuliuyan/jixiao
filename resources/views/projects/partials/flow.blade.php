                                    ...
                                    <td>
                                        @if($flow->result == 2)
                                            <div class="sheet-item sheet-btn">
                                                @if($currentUser->ability('finance', 'project.flow.charge') && ($project->state == 1 || $project->state == 3))
                                                    <a href="{{ route('project.detail.payment', $project->id) }}" class="btn btn-primary">{{ config('admin.projects.operate')[$flow->result] }}</a>
                                                @else
                                                    <a href="javascript:;" class="btn btn-default" disabled="disabled">{{ config('admin.projects.operate')[$flow->result] }}</a>
                                                @endif
                                            </div>
                                        @endif
                                        ...
                                        @if($flow->result == 3)
                                            <div class="sheet-item sheet-btn">
                                                @if($currentUser->ability('file', 'project.flow.file') && $project->state > 0)
                                        ...
                                        @if($flow->result == 4)
                                            <div class="sheet-item sheet-btn">
                                                @if(!$current_leaders->isEmpty() && !empty($projectFlow->flow[1]->user_id))
                                                ...
                                                @else
                                                    <button type="button" class="btn btn-default" disabled="disabled">{{ config('admin.projects.operate')[$flow->result] }}</button>
                                                @endif
                                            </div>
                                        @endif
                                        ...
                                        @if($flow->result == 5)
                                            <div class="sheet-item sheet-btn">
                                                @if($currentUser->division == $project->division && $currentUser->ability('division_manager', 'project.flow.bonus') && $project->state == 4)
                                        ...
                                        @if($flow->result == 6)
                                            <div class="sheet-item">
                                                @if(!$current_leaders->isEmpty() && $project->state == 5)
                                                    <input type="hidden" name="user_id" value="{{ $currentUser->id }}"/>
                                                    <input type="hidden" name="result" value="{{ $flow->result }}"/>
                                                ...
                                                @else
                                                    <button type="button" class="btn btn-default" disabled="disabled">{{ config('admin.projects.operate')[$flow->result] }}</button>
                                                    <button type="button" class="btn btn-default" disabled="disabled">驳回</button>
                                                @endif
                                            </div>
                                        @endif
                                        ...
                                        @if($flow->result == 7)
                                            <div class="sheet-item">
                                                @if($currentUser->ability('chief_engineer', 'project.flow.examine') && $project->state == 6)
                                                    <input type="hidden" name="user_id" value="{{ $currentUser->id }}"/>
                                                    <input type="hidden" name="result" value="{{ $flow->result }}"/>
                                        ...
                                        @if($flow->result == 8)
                                            <div class="sheet-item">
                                                @if($currentUser->ability('general_manager', 'project.flow.final') && $project->state == 7)
                                                    <input type="hidden" name="user_id" value="{{ $currentUser->id }}"/>
                                                    <input type="hidden" name="result" value="{{ $flow->result }}"/>
                                        ...
                                        <div class="sheet-item">
                                            ...
                                            @if($flow->user_id == $currentUser->id)
                                                <div class="sheet-out">
                                                    ...
                                                    <button type="button" data-id="{{$flow->project_flow_id}}" class="btn btn-primary remark-edit">备注</button>
                                                </div>
                                            @else
                                                @if($flow->result == 3 && $currentUser->ability('chief_engineer', 'project.flow.examine'))
                                                    <div class="sheet-out">
                                                        ...
                                                        @else
                                                            <button type="button" data-id="{{$flow->project_flow_id}}" class="btn btn-primary flow-confirm">确认</button>
                                                        @endif
                                                    </div>
                                                @endif
                                            @endif
